...
class Server:
    ...
    def __init__(self) -> None:
        ...
        self.framesSocket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        self.framesSocket.bind(("0.0.0.0", self.F_PORT))
        self.clientFramesSocket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)    
    
    
    def Start(self):
        # The function gets nothing.
        # The function send a start order to the client and make it start sending frames and getting input.
        self.clientInputSocket.send("start".encode())
    def Pause(self):
        # The function gets nothing.
        # The function send a stop order to the client and make it stop sending frames and getting input.
        self.clientInputSocket.send("pause".encode())
    def Disconnect(self):
        # The function gets nothing.
        # The function sends a disconnect order to the client and closes the sockets with the client.
        self.clientInputSocket.send("disconnect".encode())
    ...
    def GetFrame(self):
        ...
        compressedFrameBytesSize = int(self.clientFramesSocket.recvfrom(1024).decode())
        return self.clientFramesSocket.recvfrom(compressedFrameBytesSize)
