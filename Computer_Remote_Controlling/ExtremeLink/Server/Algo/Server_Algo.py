...
class Server:
    ...
    def __init__(self) -> None:
        ...
        # inputThread = threading.Thread(target=)
    ...
    def PresentFrame(newFrame:Image, width:float, height:float):
...
# if __name__ == "__main__":
    ...
    # screen.geometry("960x600")
    # screen.resizable(False, False)
    # server1 = Server()

    # mutexLock = threading.Lock()
    # inputThread = threading.Thread(target=)
    # framesThread = threading.Thread(target=)

    # inputThread.start()
    # framesThread.start()

    # frameLabel = customtkinter.CTkLabel(master=screen,
    #                                     text="",
    #                                     image = Server.PresentFrame(Server.DecompressImage(Server.GetFrame())))
    # frameLabel.place(relx=0, rely=0, anchor='center')
